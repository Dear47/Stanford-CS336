...
def model_id():
    return os.environ.get(
        "CS336_ALIGNMENT_TEST_MODEL",
        str(Path(__file__).resolve().parents[1] / "data" / "a5-alignment" / "models" / "Qwen2.5-Math-1.5B"),
    )
...
def tokenizer(model_id):
    if not os.path.exists(model_id):
        pytest.skip(f"test tokenizer/model path is unavailable: {model_id}")
    try:
